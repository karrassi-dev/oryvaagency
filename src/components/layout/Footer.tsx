...
export async function Footer() {
  ...
  const footerLinks = {
    ...
    [t('groupServices')]: [
      { label: t('linkWebDev'),   href: '/services#web-development' as const },
      { label: t('linkSEO'),      href: '/services#seo'             as const },
      { label: t('linkMarketing'),href: '/services#marketing'       as const },
      { label: t('linkAI'),       href: '/services#ai-automation'   as const },
    ],
    [t('groupContact')]: [
      { label: t('linkEmail'),   href: 'mailto:hello@oryva.agency' as const, external: true },
      { label: t('linkBookCall'),href: '/contact' as const, highlight: true },
    ],
  }
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-12">
            ...
            <div className="flex items-center gap-3 mt-6">
              {socialLinks.map(({ href, label, svg }) => (
                <a
                  ...
                  aria-label={label}
                  className="w-9 h-9 rounded-lg border border-zinc-800 flex items-center justify-center text-zinc-500 hover:text-zinc-200 hover:border-zinc-600 transition-colors"
                >
                  {svg}
                </a>
              ))}
          ...
          {Object.entries(footerLinks).map(([group, links]) => (
            <div key={group}>
              <p className="text-zinc-50 text-sm font-semibold mb-4">{group}</p>
        ...
        <div className="mt-12 pt-8 border-t border-zinc-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-zinc-500 text-sm">{t('copyright', { year })}</p>
          <p className="text-zinc-600 text-xs">{t('builtWith')}</p>
        </div>
      </div>
    </footer>
  )
